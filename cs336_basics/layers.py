...
class Embedding(torch.nn.Module):
    ...
    def forward(self, token_ids: torch.Tensor) -> torch.Tensor:
        ...
        # Sequence of token_id values for every sequence in batch, flattened into a rank-1 tensor
        tokens_flattened = rearrange(token_ids, "batch_size sequence_length -> (batch_size sequence_length)")

        # list of embeddings shape (sequence_length * batch_size, d_model), rank-2
        embeddings_indexed = torch.index_select(self.embedding_matrix, 0, tokens_flattened)

        # rearrange to rank-3 shape (batch_size, sequence_length, d_model)
        # return rearrange(
        #     embeddings_indexed,
